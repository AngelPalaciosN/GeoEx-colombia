...
export default function Mapa({ onClose }) {
  ...
  const handleSubmit = () => {
    ...
    if (!correct) {
      // Añadir clase para "eliminar" visualmente el departamento correcto
  ...
  const handleNextDepartment = () => {
    ...
    // Resetear el estilo del departamento correcto
    const correctDepartmentId = departmentIds[targetDepartment];
    const correctElement = document.getElementById(correctDepartmentId);
    if (correctElement) {
        ...
        <div className="game-info">
          ...
          {showResult && (
            <div className={`result ${isCorrect ? 'correct' : 'incorrect'}`}>
              {isCorrect 
                ? "¡Correcto!" 
                : `Incorrecto. El departamento correcto es ${targetDepartment}. Elegiste ${selectedDepartment}.`}
            </div>
          )}
        </div>
